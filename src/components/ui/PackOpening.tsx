...
const PACK_TYPE_ARTWORK = illustrationAssets.packs

const CONTENT_TYPE_ARTWORK: Record<string, string> = {
  FateSeal: illustrationAssets.consumables.fateSeal,
...
const RARITY_COLORS: Record<string, string> = {
  ...
  legendary:
    'border-purple-500 bg-gradient-to-b from-purple-500/20 to-purple-600/20',
}
      ...
      {isSelected && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10">
          <div className="w-10 h-10 bg-[var(--color-golden-yellow)] rounded-full flex items-center justify-center shadow-lg">
            <svg
              className="w-6 h-6 text-[var(--color-dark-forest)]"
              fill="none"
              stroke="currentColor"
              strokeWidth={3}
              viewBox="0 0 24 24"
...
export function PackOpening({
  ...
  const trail = useTrail(openingPack?.contents.length || 0, {
    ...
    to: {
      opacity:
        openingPack?.phase === 'selecting' || openingPack?.phase === 'confirmed'
          ? 1
          : 0,
      y:
        openingPack?.phase === 'selecting' || openingPack?.phase === 'confirmed'
  ...
  const packInfo = useMemo(() => {
    ...
    return {
      ...
      artwork: PACK_TYPE_ARTWORK[openingPack.pack.type],
    }
  }, [openingPack])
    ...
    >
      ...
      >
        ...
        <div className="text-center mb-6">
          <animated.img
            src={packInfo.artwork}
            alt=""
            aria-hidden="true"
            className="game-illustration mx-auto mb-2 h-24 w-24 object-contain"
            style={{
          ...
          <div className="text-center mb-4">
            <p className="text-[var(--color-beige-white)]">
              {openingPack.selectedIndices.length > 0 ? (
                ...
                <span className="text-[var(--color-metallic-gold)]">
                  Select up to {openingPack.maxSelections} item
                  {openingPack.maxSelections > 1 ? 's' : ''}
                </span>
              )}
            </p>
          </div>
...
export function PackCard({
  ...
  const artwork = PACK_TYPE_ARTWORK[packType]
  ...
  return (
    ...
    >
      ...
      <img
        ...
        alt=""
        aria-hidden="true"
        className="game-illustration mx-auto mt-2 h-16 w-16 object-contain"
        draggable={false}
      />
