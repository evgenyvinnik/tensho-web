...
import { FateSealSystem, getFateSealsByRarity } from './FateSealSystem'
import {
  CelestialOrbSystem,
...
export class BlessingPackSystem {
  ...
  /** Build selectable contents for the exact packs generated by the Tea House. */
  generateOfferingsForPacks(
    packs: BlessingPack[],
    options: PackGenerationOptions = {}
  ): PackOffering[] {
    this.currentOfferings = packs.map((pack) => ({
      pack,
  ...
  private generateSingleContent(
    ...
    switch (packType) {
      ...
      case 'Celestial':
        return this.generateCelestialOrbContent(
          rarity,
          existingContents,
          options.preferredYaku
        )
      case 'Tile':
  ...
  private generateCelestialOrbContent(
    ...
    const preferredOrb = preferredYaku
    ...
    if (preferredOrb && !alreadyHasPreferred) {
      return {
        ...
        rarity: preferredOrb.rarity.toLowerCase() as PackContent['rarity'],
        data: CelestialOrbSystem.createCelestialOrbInstance(preferredOrb),
      }
    }
  ...
  private generateTileContent(
    ...
    switch (selected.modifier) {
      case 'bonus':
        tile = tile.withEnhancement(EnhancementType.Bonus)
        break
      case 'mult':
        tile = tile.withEnhancement(EnhancementType.Mult)
        break
      case 'gold':
        tile = tile.withEnhancement(EnhancementType.Gold)
        break
      case 'jade':
        tile = tile.withEnhancement(EnhancementType.Wild)
        break
      case 'foil':
        tile = tile.withEdition(EditionType.Foil)
        break
      case 'holographic':
        tile = tile.withEdition(EditionType.Holographic)
        break
      case 'polychrome':
        tile = tile.withEdition(EditionType.Polychrome)
        break
      case 'negative':
        tile = tile.withEdition(EditionType.Negative)
        break
    }
