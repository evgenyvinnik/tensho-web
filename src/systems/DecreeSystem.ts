...
export class DecreeSystem {
  ...
  calculateDiscardGold(discardCount: number = 1): number {
    return this.getActiveDecrees().reduce((total, decree) => {
      if (decree.effect.type !== 'gold' || decree.effect.trigger !== 'OnDiscard') {
        return total
      }

      return total + decree.effect.amount * (decree.effect.perTile ? discardCount : 1)
    }, 0)
  }
  ...
  calculateRoundEndGold(): number {
    return this.getActiveDecrees().reduce((total, decree) => {
      if (decree.effect.type !== 'gold' || decree.effect.trigger !== 'OnRoundEnd') {
        return total
      }

      return total + decree.effect.amount
    }, 0)
  }
